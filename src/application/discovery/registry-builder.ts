...
import { buildLinkedViewContext, deriveCoverImage } from './registry-builder-helpers';
...
export class RegistryBuilder {
  ...
  async buildFromDiscovery(
    ...
    // Drop entries the unification merged away (their id now aliases the survivor).
    const unified = entries.filter((e) => registry.get(e.notionId)?.notionId === e.notionId);
    return { entries: unified, discoveryComplete: result.complete, unresolvedViews };
  }

...
import { PRO_INSTALL_DISCLOSURE } from '../shared/pro-features';
...
export class DashboardView extends ItemView {
  ...
  private renderTierBanner(container: HTMLElement): void {
    const banner = container.createDiv({ cls: 'n2o-dash-tier-banner is-caution' });

    const iconEl = banner.createSpan({ cls: 'n2o-dash-tier-icon' });
    setIcon(iconEl, 'alert-triangle');

    const body = banner.createDiv({ cls: 'n2o-dash-tier-body' });
    body.createSpan({ cls: 'n2o-dash-tier-title', text: 'Use Lite with caution.' });
    body.createSpan({
      cls: 'n2o-dash-tier-sub',
      text: ' No maintenance schedule, and your Obsidian edits never leave this disk.',
    });

    const btn = banner.createEl('button', { cls: 'n2o-dash-tier-cta n2o-cta-glow' });
    setIcon(btn.createSpan({ cls: 'n2o-cta-icon' }), 'download');
    btn.createSpan({ cls: 'n2o-cta-label', text: 'Install Pro, free for 14 days' });
    // One click installs. The dialog still opens, because it is what carries
    // the download disclosure, the progress and any failure, and acting with
    // none of those on screen would be the wrong kind of one-click.
    btn.addEventListener('click', () => this.callbacks?.onOpenUpgrade(true));

    // The banner's button starts the download on one click, so the disclosure
    // belongs here as well as in the dialog. Same sentence, one definition.
    banner.createDiv({ cls: 'n2o-dash-tier-note', text: PRO_INSTALL_DISCLOSURE });

    // Outside the box on purpose. The box is a warning about Lite; this link is
    // about the Pro offer, so it sits under the banner as a footnote to the
    // button rather than as another line of the caution.
    const more = container.createEl('a', {
      cls: 'n2o-dash-tier-more',
      text: 'Read more about N2O Sync Pro',
      href: '#',
    });
    more.addEventListener('click', (e) => {
      e.preventDefault();
      this.callbacks?.onOpenUpgrade(false);
    });
  }

...
export class ConnectFlowRenderer {
  ...
  private renderHero(root: HTMLElement, d: DashboardData): void {
    ...
    root.createDiv({
      ...
      text: 'Pull your Notion pages into this vault as markdown, with three-way merge, media, and database pages.',
    });
    ...
    this.renderAction(root, d);
    this.renderWhatYouGetCard(root);
  ...
  private renderWhatYouGetCard(root: HTMLElement): void {
    const card = root.createDiv({ cls: 'n2o-dash-whatyouget' });
    card.createDiv({ cls: 'n2o-dash-whatyouget-label', text: 'What you get' });
    const features: Array<{ n: string; t: string; s: string }> = [
      { n: '01', t: 'One-way sync', s: 'Pull Notion pages into your vault as markdown.' },
      { n: '02', t: 'Safe merges', s: 'Three-way merge, never overwrites.' },
      { n: '03', t: 'Rich content', s: 'Images, files, and database pages.' },
    ];
    for (const f of features) {
      ...
      body.createDiv({ cls: 'n2o-dash-feat-sub', text: f.s });
    }
  }
  ...
  private renderTrialNote(root: HTMLElement): void {
    ...
    info.createSpan({ text: 'N2O Sync Lite is free. One-way sync, Notion to Obsidian.' });
  }
  ...
  private startOAuthPoll(): void {
    ...
    this.oauthPollInterval = window.setInterval(() => {
      ...
      if (this.oauthSecondsLeft <= 0) {
        this.stopOAuthPoll();
        this.connectBannerText = 'OAuth timed out. Try again or use a manual token.';
        this.setStep('hero');
        return;
      }
  ...
  private updateCountdownText(): void {
    ...
    const sec = s % 60;
    this.oauthCountdownEl.textContent = `Timeout in ${min}:${sec.toString().padStart(2, '0')}`;
  }

...
export function renderConnectionTab(container: HTMLElement, ctx: SettingsTabContext): void {
  ...
  if (!isOAuth || !hasToken) {
    ...
    new Setting(oauthAltGroup)
      ...
      .setDesc('Opens Notion in your browser to authorize N2O')
      .addButton((btn) =>
        btn
          .setButtonText('Connect to Notion')
          .setCta()
          .onClick(() => {
            ctx.plugin.startOAuthFlow();
          }),
      );
  }
  ...
  aboutText.appendText(
    'N2O Sync Lite is the free edition of N2O Sync. It syncs one way, Notion to Obsidian, ' +
      'up to 100 pages. ',
  );
